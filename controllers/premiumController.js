...
exports.updatePremiumStatus=async(req,res)=>{
  try {
      ...
      if(user){
        ...
        await t.commit();
        res.status(201).json({Message:'Premium status updated successfully in DB'})
      }else{
        await t.rollback();
        res.status(400).json({Error:'User not found to Update Premium Status'})
      }
      
  } catch (error) {
      await t.rollback();
      res.status(500).json({Error:'Error updating premiumStatus in DB'})
  }
};
...
exports.getLeaderboard=async(req,res)=>{
  try {
    ...
    res.status(200).json(users);
      ...
      res.status(500).json({Error:'Error  Displaying Users in leaderboard'})
  }
}
